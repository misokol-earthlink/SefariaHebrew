...
/* ============================================================
   Pocket Torah Options modal
   Selection and calculation only. Playback controls remain
   placeholders until the next implementation step.
   ============================================================ */

let ptModalCalculationState = "not-calculated";
let ptModalPreparedReading = null;
let ptModalCalculationSerial = 0;

function setPtModalStatus(state, message) {
  ptModalCalculationState = state;

  const statusElement = document.getElementById("ptProcessingStatus");
...
function clearPtModalReferenceDisplay() {
  [
    "ptBookDisplay",
    "ptStartChapterDisplay",
    "ptStartVerseDisplay",
    "ptEndChapterDisplay",
    "ptEndVerseDisplay"
  ].forEach(function(id) {
...
function displayPtModalSelection(selection) {
  const values = {
    ptBookDisplay: selection.book,
    ptStartChapterDisplay: selection.startChapter,
    ptStartVerseDisplay: selection.startVerse,
    ptEndChapterDisplay: selection.endChapter,
    ptEndVerseDisplay: selection.endVerse
  };

  Object.keys(values).forEach(function(id) {
    const element = document.getElementById(id);
    if (element) {
      element.textContent = String(values[id]);
    }
  });
}

function getPtSelectedReadingType() {
  const selected =
    document.querySelector('input[name="ptReading"]:checked');

  return selected ? selected.value : "full";
}

function loadPocketTorahAudioDurationForSefaria(audioPath) {
  return new Promise(function(resolve, reject) {
    ...
    audio.addEventListener("loadedmetadata", function() {
      ...
      if (Number.isFinite(duration)) {
        ...
        reject(new Error("Pocket Torah audio duration is unavailable."));
      }
    }, { once: true });

    audio.addEventListener("error", function() {
      cleanup();
      reject(new Error("Pocket Torah audio metadata could not be loaded."));
    }, { once: true });
...
async function recalculatePtModalSelection() {
  const parshaSelect = document.getElementById("ptParshaSelect");
  if (!parshaSelect) return;

  const parshaName = parshaSelect.value;
  const readingType = getPtSelectedReadingType();

  ptModalPreparedReading = null;
  clearPtModalReferenceDisplay();

  if (!parshaName) {
    setPtModalStatus("not-calculated", "Not calculated");
    return;
  }

  const calculationSerial = ++ptModalCalculationSerial;
  setPtModalStatus("calculating", "Calculating...");

  try {
    const prepared =
      await PocketTorah.prepareReadingTiming(
        parshaName,
        readingType,
        loadPocketTorahAudioDurationForSefaria
      );

    if (calculationSerial !== ptModalCalculationSerial) {
      return;
    }

    ptModalPreparedReading = prepared;
    displayPtModalSelection(prepared.selection);
    setPtModalStatus("calculated", "Calculated");

    console.log("Pocket Torah modal selection:", prepared.selection);
    console.log(
      "Pocket Torah modal playback timing:",
      prepared.playbackSegments
    );
  } catch (error) {
    if (calculationSerial !== ptModalCalculationSerial) {
      return;
    ...
    console.error("Pocket Torah modal calculation failed:", error);
    ptModalPreparedReading = null;
    clearPtModalReferenceDisplay();
    setPtModalStatus("error", "Calculation unavailable");
  }
}

async function initializePocketTorahModal() {
  const parshaSelect = document.getElementById("ptParshaSelect");
  if (!parshaSelect || typeof PocketTorah === "undefined") {
    return;
  }
  ...
  try {
    await PocketTorah.ensureResourcesLoaded();

    const parshaNames = PocketTorah.getParshaNames();
    ...
    setPtModalStatus("error", "Pocket Torah data unavailable");
    return;
  }

  parshaSelect.addEventListener("change", function() {
    recalculatePtModalSelection();
  });
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", initializePocketTorahModal);
} else {
  initializePocketTorahModal();
}
